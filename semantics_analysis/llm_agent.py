    ...
    def __call__(self, prompt: str, stop_sequences: List[str], max_new_tokens: int) -> str:
        ...
        while True:
            try:
                ...
                if attempt >= len(values) or not self.use_all_tokens:
                    raise e

                attempt += 1
                self.token_idx += 1
                self.token_idx = self.token_idx % len(values)
                self.llm = InferenceClient(model=self.model, timeout=8, token=values[self.token_idx])
                continue
